...
struct PhongShader : IShader {
    ...
    virtual std::pair<bool,TGAColor> fragment(const vec3 bar) const {
        ...
        vec3 p = normalized(bar[0] * tri[0] + bar[1] * tri[1] + bar[2] * tri[2]); // triangle normal in eye coordinates
        vec3 n = normalized(bar[0] * norml_[0] + bar[1] * norml_[1] + bar[2] * norml_[2]);
